...
tabs.initialize = function (triggers, panels) {
  ...
  for (tabs.i = 0; tabs.i < triggers.length; tabs.i++) {
    triggers[tabs.i].addEventListener('click', function (e) {
      ...
      var oldLabel = document.querySelector("hero-stats").shadowRoot.querySelector('.hero-stats-tabs-item.active');
      var newLabel = e.target.parentElement;

      var oldButton = oldLabel.children[0];
      var newButton = newLabel.children[0];

      var oldTab = document.querySelector("hero-stats").shadowRoot.querySelector('.hero-stats-slide[aria-labelledby="' + oldButton.attributes['aria-controls'].value + '"]');
      var newTab = document.querySelector("hero-stats").shadowRoot.querySelector('.hero-stats-slide[aria-labelledby="' + newButton.attributes['aria-controls'].value + '"]');
